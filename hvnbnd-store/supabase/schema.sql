...
create table if not exists settings (
  ...
  free_shipping_threshold numeric not null default 75.00,
  updated_at timestamptz default now(),
  constraint single_row check (id = 1)
);
insert into settings (id, shipping_rate, free_shipping_threshold) values (1, 5.00, 75.00) on conflict (id) do nothing;
