...
export async function GET() {
  const { data, error } = await supabase.from('settings').select('shipping_rate').eq('id', 1).single();
  if (error) {
    // Fail safe with a sane default rather than breaking checkout entirely.
    return NextResponse.json({ shipping_rate: 5 });
  }
