...
export default function AdminDashboard() {
  ...
  async function updateShipping(rate) {
  ...
  return (
    <div className="admin-panel">
      ...
      <p style={{ fontSize: 13, color: 'var(--stone-dark)', lineHeight: 1.6, marginBottom: 16 }}>
        This is the flat shipping fee charged on every order at checkout. Update it here any time —
        it applies immediately, no redeploy needed.
      </p>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <span className="field-label" style={{ marginBottom: 0 }}>Flat rate ($)</span>
        <input
          type="number"
          step="0.01"
          min="0"
        ...
        />
        <button className="mini-btn" onClick={() => onUpdateShipping(Number(rate))}>Save</button>
      </div>
    </div>
  );
}
